...
from PyQt5.QtWidgets import QApplication, QTableWidgetItem, QPushButton, QButtonGroup, QWidget, QTableWidget, QLabel, QFrame
...
class YahtzeeGui(object):
    ...
    def setup(self, main_window):
        ...
        icon = QIcon()
        icon.addPixmap(QPixmap(os.path.join(self.resources_path, "y6.jpg")), QIcon.Normal, QIcon.Off) #icon for menu bar
        ...
        for i in range(5):
            ...
            self.die.setIcon(QIcon(os.path.join(self.resources_path, self.die_set.get_at(i).icon)))
        ...
        for i in range(20):
            ...
            if i < 7:  # first rows get icons in the header
                icon = QIcon()
                icon.addPixmap(QPixmap(os.path.join(self.resources_path, "w" + str(i) + ".jpg")), QIcon.Normal, QIcon.Off)
                item.setIcon(icon)
            item = QTableWidgetItem()
    ...
    def yahtzee_table_click(self, item):
        ...
        if self.auto_fill == False: # checks to see if a yahtzee bonus autofill has already occurred
            if self.yahtzee_bonus > 2 and self.die_set.get_at(0).pip == self.die_set.get_at(1).pip == self.die_set.get_at(2).pip == self.die_set.get_at(3).pip == self.die_set.get_at(4).pip:
                ...
                if len(column_list) < 3: # if the column's upper section pip rows are not already all filled in
                    ...
                    self.y_table_list.append((pip_value, entry_column, point_value))  # append the tuple with row, col and total
            ...
            if in_table_list == False and self.auto_fill == False: # next line makes sure that the player didn't click on a "total" cell
                ...
                if row >= 0 and row <= 5:
                    for i in range(5):
                        if self.die_set.get_at(i).pip == (row + 1): # if the die pip matches the row selected
                ...
                if row >= 10 and row <= 16:
                    ...
                    sorted_dice_group.sort(key=lambda x: x.pip) #sort the group based upon pip value
                    for i in range(5):
    ...
    def three_of_a_kind(self):
        for i in range(3): #look at the first three dice that have been sorted
            if self.sorted_dice_list[i] == self.sorted_dice_list[i + 2]: # if the first is equal to the third then three of a kind
                total = 0
                for j in range(5):
                    total += self.sorted_dice_list[j]
                return total # NOTE return ends the method so no need to try to break out
        return 0 # if no three of a kind return a zero

    def four_of_a_kind(self):
        for i in range(2):
            if self.sorted_dice_list[i] == self.sorted_dice_list[i + 3]: #if the first is equal to the fourth then four of a kind
                total = 0
                for j in range(5):
                    total += self.sorted_dice_list[j]
                return total
    ...
    def full_house(self): #checks if the first three and last two are the same and first two and last three sorted dice
        if self.yahtzee_joker == True:
            return 25
        if self.sorted_dice_list[0] == self.sorted_dice_list[2] and self.sorted_dice_list[3] == self.sorted_dice_list[4] or self.sorted_dice_list[0] == \
    ...
    def small_straight(self):
        ...
        for i in range(step_to): # check and see if each die is one more than the preceeding die
            if new_list[i + 3] == (new_list[i + 2] + 1) and new_list[i + 2] == (new_list[i + 1] + 1) and new_list[i + 1] == (
                    new_list[i] + 1):
                return 30
        return 0
    ...
    def large_straight(self): #same logic as above but all five must be in sequence
        if self.yahtzee_joker == True:
            return 40
        if self.sorted_dice_list[4] == (self.sorted_dice_list[3] + 1) and self.sorted_dice_list[3] == (self.sorted_dice_list[2] + 1) and \
                self.sorted_dice_list[2] == (self.sorted_dice_list[1] + 1) and self.sorted_dice_list[1] == (self.sorted_dice_list[0] + 1):
            return 40
        else:
            return 0

    def yahtzee(self): # just checks if all die pip values are the same
        if self.sorted_dice_list[0] == self.sorted_dice_list[1] == self.sorted_dice_list[2] == self.sorted_dice_list[3] == self.sorted_dice_list[4]:
    ...
    def chance(self): # just adds all five die pip values together
        total = self.sorted_dice_list[0] + self.sorted_dice_list[1] + self.sorted_dice_list[2] + self.sorted_dice_list[3] + self.sorted_dice_list[4]
        return total

    def die_clicked(self, button): # this is called if a die is clicked
        clicked_die = self.button_die[button]  # sets clickedDie = to the die object dictionary pair to the button pressed
        if clicked_die.is_clicked == 1: # toggles the is_clicked value
            clicked_die.is_clicked = 0
            clicked_die.icon = "w" + str(clicked_die.pip) + ".jpg" # makes the die white
        else:
            clicked_die.is_clicked = 1
            clicked_die.icon = "y" + str(clicked_die.pip) + ".jpg" # makes the die yellow
        for i in range(5):
            if self.button_list[i] == button:
                self.button_list[i].setIcon(QIcon(os.path.join(self.resources_path, clicked_die.icon)))
    ...
    def new_game(self): # helper function to start a new game
        ...
        for i in range(5): # reset all of the die objects to 0 and white
            ...
            die_object.icon = "w0.jpg"
            die_object.is_clicked = 0
            self.button_list[i].setIcon(QIcon(os.path.join(self.resources_path, die_object.icon)))
        ...
        if self.game_over == False: # if not in a game over state
            if self.dice_rolled == False: # if the dice have not been rolled yet
                ...
                for i in range(5): # since this is the first roll turn all yellow dice to white and
                    ...
                    die.icon = "w" + str(die.pip) + ".jpg"
                    die.is_clicked = 0 # reset the is_clicked value to 0
                    self.button_list[i].setIcon(QIcon(os.path.join(self.resources_path, die.icon))) # update the buttons tha actually display the dice
            ...
            if self.die_rolls < 3: # if not pas the end of dice rolls
                for i in range(10): # we're goining to go through 10 different dice values to simulate a roll
                    for j in range(5): # go through each die
                        if self.die_set.get_at(j).is_clicked == 0: # if the die can be rolled (ie not clicked)
                ...
                _translate = QtCore.QCoreApplication.translate
                if self.die_rolls == 2: # if there is one roll left mahe roll singular
    ...
    def show_die(self, die_object, rolling_die, last_time): #helper method to display the dice
        ...
        die_object.icon = "w" + str(pip) + ".jpg" #set the icon image
        self.button_list[rolling_die].setIcon(QIcon(os.path.join(self.resources_path, die_object.icon)))
        QApplication.processEvents() # again must be called to update the icon images
        if last_time == 1: # if this is the last roll then
            die_object.pip = pip # set the die object's pip value to the random number for use later
